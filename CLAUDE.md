...
AI 协作的目标是**最大化产出质量与速度**,不是最大化角色分工的仪式感。协作规则以**任务特征**为准,不以"前端/后端"这种粗粒度划分为准。

### 7.2 角色与分工

**Claude(主导者)**:
- 需求分析、架构设计、任务拆分、代码审查、最终验收、Git 提交管理
- **可以直接编写代码**的场景:
  - 代码审查中发现的小范围修复(< 50 行)
  - 类型补全、注释补全、测试补齐
  - 紧急热修(Codex / Gemini 不可用且阻塞用户)
  - 跨模块粘合代码、配置文件调整
- **应该委派**的场景:
  - 新功能完整实现(页面、服务、数据层)
  - 大范围重构
...
**Codex / Gemini(执行者)**:
- 按任务能力画像派发,不按前后端划分
- 能力画像维度:强类型推理、长上下文、视觉对齐、测试生成、重构改造
- 派发时在任务描述中说明"为什么选这个模型",便于后续复盘

### 7.3 任务派发决策

派发前先判断任务特征,参考下表选择执行者:

| 任务特征 | 优先执行者 | 备注 |
|---------|-----------|------|
| 跨文件大重构 | Codex | 长上下文 + 结构化改动能力强 |
| 新页面/新组件初版 | Gemini | 视觉对齐与交互实现 |
| 复杂业务逻辑 + 多轮迭代 | Codex | 类型推理与调试更稳 |
| Review 中的小修 | Claude | 避免委派开销 |
| 测试用例补齐 | Codex / Gemini 皆可 | 按现有代码归属派发 |
| 紧急线上热修 | Claude | 不走委派,直接修 |

能力画像会随模型版本变化,每季度复盘一次本表。

### 7.4 降级机制

- **Codex 不可用** → Gemini 接管,任务描述标注"降级接管"
- **Gemini 不可用** → Codex 接管,同上标注
- **两者都不可用** → Claude 直接实现,但必须自我 review 或下一次开工前补 review
- 降级不是失败,是协作弹性的一部分

### 7.5 前端设计流程

Stitch 定位为**视觉灵感参考**,不是实现输入。前端流程分两类:

**有明确设计稿的页面**:
- 设计稿(Figma / Stitch 导出)→ 提取 component spec(props / 状态 / 交互说明)→ Gemini 实现 → Claude 校准

**无设计稿的内部工具页**:
- Claude 直接写 component spec → Gemini 实现 → Claude 校准

**禁止**让 Gemini 基于 Stitch HTML/Tailwind 稿直接翻译到 Vue——双重翻译损耗大,视觉一致性反而更难保证。
...
派发时必须说明:任务目标、涉及文件、验收标准、已知约束。模糊的 prompt 产出模糊的代码。
