...
@router.get("/calendar")
def get_calendar(request: Request) -> dict[str, object]:
    calendar = _svc(request).get_calendar()
    return ok_response(calendar.model_dump(mode="json"))
...
@router.post("/plans/{plan_id}/precheck")
...
@router.get("/plans/{plan_id}/receipts")
def list_receipts(plan_id: str, request: Request) -> dict[str, object]:
    receipts = _svc(request).list_receipts(plan_id)
    return ok_response([receipt.model_dump(mode="json") for receipt in receipts])


@router.get("/plans/{plan_id}/receipt")
def get_latest_receipt(plan_id: str, request: Request) -> dict[str, object]:
    receipt = _svc(request).get_latest_receipt(plan_id)
