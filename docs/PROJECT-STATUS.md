...
> 更新日期：2026-07-04（当前应用版本以根 `package.json#version` 为准；本次复核已对齐 M05 创作链路汇入、受管轨道装配、默认 9:16 预览、选中片段真实媒体预览、本地预检状态链路、智能粗剪建议审阅门禁与工作台布局分型）
...
| 5 | `ai_editing_workspace` | AI 剪辑工作台 | 已落地 | 已接线 | 已登记 | 进行中 | 已迁移为独立全宽工作台布局，不再受普通内容页最大宽度限制；预览区默认 9:16 且可切换 16:9，结构预览可拖动播放头，紧凑窗口保留最小播放控制；脚本、分镜、配音、字幕可汇入三条等长受管轨道并保留手动轨道；clip 移动 / 裁剪 / 替换 / 插入 / 分割 / 删除、M05 到 M14 的 `timeline_id` 交接、选中资产片段真实媒体预览已落地；智能粗剪已从“生成后直接修改时间线”改为“生成建议草稿、右侧基础属性面板审阅、用户确认后原子应用”，支持 selected/all apply、忽略、过期 token 拒绝、失败回滚和 `failed_parse` 可见反馈；完整时间线合成渲染、更多 AI 生成动作和真人视觉 E2E 仍待深化。 |
