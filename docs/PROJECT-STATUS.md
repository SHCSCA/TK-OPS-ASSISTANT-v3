...
| 5 | `ai_editing_workspace` | AI 剪辑工作台 | 已落地 | 已接线 | 已登记 | 进行中 | 已迁移为独立全宽工作台布局，不再受普通内容页最大宽度限制；预览区默认 9:16 且可切换 16:9；脚本、分镜、配音、字幕可汇入三条等长受管轨道并保留手动轨道；clip 移动 / 裁剪 / 替换 / 插入 / 分割 / 删除、M05 到 M14 的 `timeline_id` 交接、选中资产片段真实媒体预览已落地；完整时间线合成渲染、更多 AI 生成动作和真人视觉 E2E 仍待深化。 |
