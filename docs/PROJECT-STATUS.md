...
> 更新日期：2026-07-04（当前应用版本以根 `package.json#version` 为准；本次复核已对齐 M05 创作链路汇入、受管轨道装配、默认 9:16 预览、选中片段真实媒体预览、本地预检状态链路与工作台布局分型）
...
| 5 | `ai_editing_workspace` | AI 剪辑工作台 | 已落地 | 已接线 | 已登记 | 进行中 | 已迁移为独立全宽工作台布局，不再受普通内容页最大宽度限制；预览区默认 9:16 且可切换 16:9，结构预览可拖动播放头，紧凑窗口保留最小播放控制；脚本、分镜、配音、字幕可汇入三条等长受管轨道并保留手动轨道；clip 移动 / 裁剪 / 替换 / 插入 / 分割 / 删除、M05 到 M14 的 `timeline_id` 交接、选中资产片段真实媒体预览已落地；失败回滚会提示已恢复到操作前时间线；完整时间线合成渲染、更多 AI 生成动作和真人视觉 E2E 仍待深化。 |
...
3. **深度功能分阶段打通**：M05 已接通创作链路汇入、受管轨道装配、基础 clip 原子操作、选中片段真实媒体预览与 M14 来源时间线交接；M07 配音中心已接通火山豆包 TTS2 真实合成与音频预览；M06 / M08 的 FFmpeg 深度解析、字幕对齐与时间线回流仍需继续逐段落地。
