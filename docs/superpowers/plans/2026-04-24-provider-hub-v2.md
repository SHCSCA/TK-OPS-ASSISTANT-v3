...
5. 视频生成 Provider、资产分析 Provider 作为独立可配置 Provider 进入统一接入链路，不再只停留在默认占位模型。
...
- 远端模型缺少 modalities 时，Runtime 按 Model ID / 展示名推断视频、TTS、视觉等基础类型，例如 `doubao-seedance-*` 必须归入 `video_generation`。
- 连接检查发现 404 / 无权限模型时，将该模型写入停用覆盖记录，并从模型目录和能力矩阵可选项中隐藏。
...
- 字幕对齐可以选择文本生成模型；资产分析可以选择具备视觉输入并输出文本的模型。
- 无权限模型经过健康检查后不再出现在 Provider 模型目录和能力配置下拉里。
