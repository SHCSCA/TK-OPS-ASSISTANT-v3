...
桌面主窗口不再允许无限缩小。Tauri 主窗口默认尺寸为 `1680 × 960`，最小尺寸为 `1680 × 900`。这个下限覆盖展开侧栏、M05 工作台、底部时间线、状态栏和右侧 Detail Panel 的基本可用空间，避免继续缩小时出现列折叠、控件重叠或时间线挤压。

右侧 Detail Panel 打开时，壳层使用 `max-width: 1859px` 作为保护阈值：在该阈值及以下自动把左侧导航视为有效折叠态，释放工作区宽度；从 `1860px` 起允许左侧展开侧栏和右侧抽屉同时存在。该策略只改变壳层占位，不改变用户真实的侧栏偏好。
